...
$level = $_SESSION['levelx'];
  ...
  $sql = "UPDATE caseDis SET dateVDOCall = null WHERE dateVDOCall < now() ";
  $mysqli->query($sql);

$q1="SELECT * FROM member where hospcode='$d' and idMeet like 'https://%' ";          
...
<table class="table table-bordered table-striped">
    <thead>
		<tr>
	    	...
	    	<th class='text-center'><font size=2>เบอร์โทร อปท.</th>
	    </tr>
    </thead>
    <tbody>
    	<?php 
			$sql = "SELECT * FROM caseDis where hospcode='$d' and dateVDOCall >now() order by dateVDOCall";
    		...
    		while($row = mysqli_fetch_array($result)) 
    		{
				$cid=base64_encode($row['cid']);
					...
					else{$showVDO="dateTimex($dateVDOCall);?><a href='https://meet.jit.si/<?=$dMeet?>'' target='_blank' class='btn btn-info' role='button'><i class='fa fa-video-camera'></i></a>";}
	?>
    	<tr>
	        <td><?php echo $i; ?></td>
			...
			if($dateVDOCall==null){echo "<td>ไม่มีนัดหมาย</td>";}
				...
				?>
			<td> <?php echo dateTimex($dateVDOCall);?><a href="https://meet.jit.si/<?=$dMeet?>" target="_blank" class="btn btn-info" role="button"><i class="fa fa-video-camera"></i></a> </td>	    	
			<?php } ?>
