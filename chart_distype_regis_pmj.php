...
$provinces = $_SESSION['province_pmj'];
...
WHERE o.provid = '$provinces' and o.off_type in('04', '06', '07')
GROUP BY o.off_id";
...
<?php
    ...
    $query = "SELECT * FROM pmj WHERE provinces = $provinces LIMIT 1";
    $result = mysqli_query($conn, $query);
    $row = mysqli_fetch_array($result);
    $pmj = $row['pmj_name'];

    $resultx = $mysqli->query("SELECT * from caseDis WHERE statusCase='PMJ' ");
    $countPMJ=mysqli_num_rows($resultx);
    $resultx = $mysqli->query("SELECT * from caseDis WHERE statusCase='OBT' or infoFrom='OBT' ");
    $countOBT=mysqli_num_rows($resultx);
    $resultx = $mysqli->query("SELECT * from caseDis WHERE statusCase='DOC' and infoFrom='PMJ' ");
    $countDOC=mysqli_num_rows($resultx);
    $resultx = $mysqli->query("SELECT * from caseDis WHERE statusCase='OK' ");
    ...
    <!--**********************************        Nav header start       ***********************************-->
        ...
        <div class="header">    
            <div class="header-content clearfix ">
                ...
                <p><h3 class="text-green"> &nbsp;&nbsp;&nbsp;    ระบบบริการข้อมูลคนพิการ  <font color="gray" size="2">&nbsp;&nbsp;&nbsp; Nongbualamphu Disability Data Center</font></h3><h5>&nbsp;&nbsp;&nbsp;หน่วยงาน: <?php echo $pmj;?></h5>
                </div>
